...
class Add_new_product : Fragment() {
    ...
    private fun insertNewProductToDatabase() {
        ...
        //still need to validate if img_materialPhoto is empty
        val bitmap = (img_materialPhoto.drawable as BitmapDrawable).bitmap
        if(!(TextUtils.isEmpty(materialName)) && (costPI != 0.0) && bitmap != null){

            val material = Material(materialId, materialName, Utils.getBytes(bitmap), 0, costPI, 0.00, 10, 1)
            lifecycleScope.launch {
                val dao: UserDao = UserDatabase.getDatabase(requireContext()).userDao()
